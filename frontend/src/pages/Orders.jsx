...
export default function Orders() {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:20, paddingBottom: selected.size > 0 ? 80 : 0 }}>
            ...
            <>
              ...
              <div style={{ position:'relative' }}>
                <div style={{ display:'flex', alignItems:'center', background:'var(--card)', border:'1px solid var(--rule)', borderRadius:9, overflow:'hidden', height:36 }}>
                  ...
                  <input className="input" placeholder="Edit order…" value={editSearch}
                ...
                {editSearchOpen && editResults.length > 0 && (
                  <div style={{ position:'absolute', top:'calc(100% + 6px)', right:0, zIndex:400, width:300, background:'var(--card)', border:'1px solid var(--rule)', borderRadius:10, overflow:'hidden', boxShadow:'0 8px 24px rgba(0,0,0,.2)' }}>
                    {editResults.map(o => (
                      <button key={o._id} type="button" onMouseDown={() => { setEditSearchOpen(false); setEditSearch(''); setEditResults([]); openEdit(o); }}
      ...
      {/* Mobile: inline action bar when items selected — always visible, no scrolling */}
      ...
      {isMobile ? (
        !selected.size && <input className="input" placeholder="Search orders…" value={filters.search}
      ...
      {deletedModal && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.65)', zIndex:600, display:'flex', alignItems:'center', justifyContent:'center', padding:'24px 16px', overflowY:'auto' }}
          ...
          <div style={{ background:'var(--card)', borderRadius:16, width:'100%', maxWidth:700, maxHeight:'75vh', display:'flex', flexDirection:'column', boxShadow:'0 16px 48px rgba(0,0,0,.4)', margin:'auto' }}
      ...
      {!isMobile && selected.size > 0 && (
        <div className="bulk-bar fade-in" style={{
          ...
          {STATUS.map(s => (
            <button key={s} onClick={() => bulkUpdateStatus(s)} disabled={bulkWorking}
              style={{ padding:'5px 10px', borderRadius:8, border:'none', cursor:'pointer', fontSize:11, fontWeight:500, background:'rgba(255,255,255,.1)', color:'rgba(255,255,255,.85)' }}>
              → {s}
            </button>
          ))}
          <button onClick={bulkDelete} disabled={bulkWorking}
